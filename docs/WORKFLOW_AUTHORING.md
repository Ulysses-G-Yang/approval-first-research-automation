...
先执行 `agent list-workflows`，即可查看每个内置工作流的摘要、前置条件和工具序列。该命令只读取仓库内的 YAML，不创建任务也不调用模型。
...
- 一句面向结果的 `summary`，避免用内部模块名代替用户目标。
- `requires`、`when` 和 `for_each` 的离线测试。
- 一个使用自建公开数据或本地样例的可复制示例。
- 明确的网络、写入、登录、发布和 Provider 边界。

不要把页面动作、任意脚本、明文密钥或平台登录放进工作流 YAML。需要外部副作用的能力必须作为经审阅插件实现，并在计划中以敏感步骤展示。
